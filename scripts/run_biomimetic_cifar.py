...
def get_blur_transform(epoch, total_epochs):
    halfway = total_epochs / 2
    if epoch >= halfway:
        sigma = 0
    else:
        progress = epoch / halfway
        sigma = 4.0 * (1 - progress)
    ...
    t_list.append(transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)))
...
def main():
    ...
    # load data
    train_set = datasets.CIFAR10(data_path, train=True, download=True, transform=transforms.ToTensor())
    
    model = KANC_CIFAR(grid_size=3).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=0.001)
    criterion = nn.CrossEntropyLoss()

    print("starting biomimetic training (Experiment Group)...")
    epochs = 20

    for epoch in range(epochs):
        # update vision
        train_set.transform = get_blur_transform(epoch, epochs)
        ...
        for i, (imgs, labels) in enumerate(loader):
            ...
            if (i + 1) % 50 == 0:
                print(f"epoch {epoch+1} | batch {i+1} | loss: {round(loss.item(), 4)} | blur active")
            
        print(f"--- EPOCH {epoch+1} FINISHED | AVG LOSS: {round(total_loss/len(loader), 4)} ---")

    save_file = os.path.join(data_path, "model_b_biomimetic.pth")
    torch.save(model.state_dict(), save_file)
    print("saved biomimetic model to:", save_file)
